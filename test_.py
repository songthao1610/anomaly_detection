...
@pytest.mark.parametrize("df_input, sensitivity_score, number_of_anomalies", [
    ...
    (anomalous_sample, 95, 13),
    (anomalous_sample, 85, 9), 
    (anomalous_sample, 75, 6),
...
@pytest.mark.parametrize("df_input, max_fraction_anomalies, number_of_anomalies", [
    ...
    (anomalous_sample, 0.2, 4), 
    (anomalous_sample, 0.3, 6),
    (anomalous_sample, 0.4, 7), 
    (anomalous_sample, 0.5, 9),   
    (anomalous_sample, 0.6, 10),  
    (anomalous_sample, 0.7, 12),
    (anomalous_sample, 0.8, 13), 
...
@pytest.mark.parametrize("df_input, function, expected_normal", [
    ...
    (larger_uniform_data, check_dagostino, False),  
